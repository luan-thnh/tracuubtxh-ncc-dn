...
function createMultipleInputGroups(inputElement, dataArray) {
  ...
  for (let i = 0; i < dataArray.length - 1; i++) {
    ...
    item.innerHTML = `
      <input type="text" class="form__input" name="${nameAttr}" value="${dataArray[i + 1]}"/>
...
btnUpdate.addEventListener('click', () => {
  ...
  if (ngaySinhInput.value.length === 4) {
  ...
  const updatedData = {
    ...
    mucBTXH: [mucBTXHInput.value, ...document.querySelectorAll('input[name="mucBTXH"]')]
      .map((input) => input.value)
      .filter(Boolean),
    mucNCC: [mucNCCInput.value, ...document.querySelectorAll('input[name="mucNCC"]')]
      .map((input) => input.value)
      .filter(Boolean),
    ghiChu: [ghiChuInput.value, ...document.querySelectorAll('input[name="ghiChu"]')]
      .map((input) => input.value)
      .filter(Boolean),
  };
...
function handleClickRemove(btnRemove) {
  btnRemove.addEventListener('click', (e) => {
    const formMultipleInput = e.target.closest('.form__multiple-input');
    formMultipleInput.removeChild(e.target.closest('.form__multiple-input-group'));
  });
}
